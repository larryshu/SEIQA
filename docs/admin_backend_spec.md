...
- **FastAPI runtime**（`app/`）：跑 agent loop，呼叫 `community_search` skill，並行 fan-out 到即時爬 Dcard（DrissionPage 過 Cloudflare；失敗退回 Qdrant 向量庫 fallback）與即時爬 PTT，綜合回答；問「比例／正反意見」時再呼叫第二個 skill `stance_breakdown` 做立場統計（LLM 逐則分類、Python 加總，前端畫圖）。
...
> **已知缺口**：`stance_breakdown` 的統計結果（`chart`）**目前沒有落地**——`message` 表沒有對應欄位，所以 `GET /conversation/{sid}` 還原對話時圖不會回來（只有前端 `localStorage` 那份備份存得回）。要跨裝置還原得補一個 `chart JSON NULL` 欄位。
...
| POST | `/api/v1/end-auth/login/` | 終端使用者登入，**回 JWT**（供 Streamlit 與 runtime `/demo` 代理） | public* |
