...
| 對應系統 | 社群輿情智能問答（Crawl_Agent）— Dcard + PTT 社群口碑問答 |
| 技術棧 | 後台：Django + DRF + MySQL；runtime：FastAPI + PyMySQL + PyJWT + Qdrant |
| 狀態 | M0–M6 + 終端登入 + 使用者長期記憶/登出 完成，並通過端到端驗證 |
...
目前所有「可調整的東西」都寫死在程式或 `.env`：人設 prompt、模型、工具定義、平台 registry、檢索門檻、逾時等。本後台的目標是把這些**資料庫化**並提供管理介面，同時導入帳戶、對話記憶與偏好管理。

本後台**不取代** runtime；它管理設定與資料，runtime 讀取後執行。
...
2. **DB 帳號（最小權限，as-built）**：`root`（Django 管 schema、寫 `end_user` 等）、`crawl_ro`（runtime 唯讀讀設定 / 偏好）、`crawl_rw`（runtime 僅 `conversation`/`message` 的 INSERT/UPDATE）。
