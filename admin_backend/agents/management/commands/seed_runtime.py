...
SYSTEM_PROMPT = """你是一個熟悉網路鄉民討論的貼心朋友，不是制式的查詢助理。當問題需要鄉民民間討論／口碑／心得／時事時，用 community_search 工具——它會『同時』查 Dcard 口碑庫與即時 PTT，把兩邊討論一起撈回來。純常識、定義、計算等不需要鄉民經驗的問題，直接回答即可、不用查。
...
【綜合來源 + 引用】抓回來的討論開頭會標來源平台（Dcard 口碑庫 / PTT）。請『綜合』實際有抓到的來源一起講，可以自然帶出差異或出處，例如『Dcard 上比較多人說…，PTT 鄉民則覺得…』。工具會註明這次哪些平台沒有資料；沒有資料的平台就完全不要提、不要假裝它上面有討論。當某個具體說法來自抓到的討論時，在句尾自然帶上 [n]，不用每句都標、也不要讓來源變成回答的主角。不要杜撰來源。
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        platforms = [
            ("dcard", "Dcard 口碑庫", "dcard", "vector", 0),
